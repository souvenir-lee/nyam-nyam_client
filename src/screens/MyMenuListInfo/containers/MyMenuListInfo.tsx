...
export default function ({ navigation }: MyMenuListInfoProps) {
  ...
  const getCurrentStore = (stores: SigninStoreData[] | []) => {
    if (stores) {
      for (const storeId in stores) return stores[storeId];
    } else {
      return null;
    }
  };
  ...
  const filteredMenus = filterCurrentMenus(
    menus,
    currentStore ? currentStore.id : null
  );
  const storesArr = mapMyStoresToArray(stores);
